...
const EquipmentManager = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingId) {
      const result = await updateEquipment(editingId, formData);
      if (result) {
        setEquipment(equipment.map(eq => eq.id === editingId ? result : eq));
        handleCloseModal();
  ...
  const handleDelete = async (id) => {
    const confirmed = await confirmAction("Are you sure about deleting this equipment?");
    if (confirmed) {
          ...
          {loading ? (
            ...
            >
              ...
              <p className="text-gray-600 dark:text-gray-400 text-lg">
                {searchTerm ? 'No equipment found' : 'No equipment yet. Add one to get started!'}
              </p>
            </motion.div>
          ) : (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Barbell, Dumbbells, Treadmill"
                  required
                />
              </div>
