...
import { confirmAction } from "../utils/ConfirmAction.jsx";
...
const Members = () => {
  ...
  const handleDeleteMember = async (enrollmentId) => {
    const confirmed = await confirmAction("Are you sure to Delete this member?");
    if (!confirmed) return;

    try {
      const res = await deleteUserFun(enrollmentId);
      if (res) {
        const data = await getMembers();
        dispatch(setUsers(data));
        setMembers(data);
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
      ...
      <div className="max-w-[95%] mx-auto py-2 sm:px-6 lg:px-8">
        <motion.div
          className="mb-8"
          ...
          <div className="overflow-x-auto overflow-y-auto max-h-[80vh]">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="sticky top-0 bg-gray-50 dark:bg-gray-700">
                <tr>
                  <th className="px-3 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                    S.No.
                  </th>
                  <th className="px-3 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                    Member
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {filteredMembers.map((member, index) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="bg-primary-100 dark:bg-primary-900/30 rounded-full mr-3">
                    ...
                    <td className="px-3 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900 dark:text-white">
                        {(member?.guardianName?.length > 30
                          ? member.guardianName.substring(0, 30) + "..."
                          : member?.guardianName) || "-"}
                      </div>
                    </td>
                    ...
                    <td className="px-3 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex flex-row gap-2 items-center justify-end">
                        {member.testDone ? (
                          <button
                            className="text-destructive hover:font-bold hover:text-green-600 py-1 px-2 bg-green-300 rounded-sm hover:scale-90 transition-all"
                            type="button"
                            onClick={() => {
                              navigate(
                                `/test/view-test/${member.enrollmentId}`,
                              );
                            }}
                          >
                            View Test
                          ...
                          <button
                            className="text-destructive hover:font-bold hover:text-red-600 py-1 px-2 bg-red-300 rounded-sm hover:scale-90 transition-all"
                            type="button"
                            onClick={() => {
                              navigate(
                                `/test/take-test/${member.enrollmentId}`,
                              );
                            }}
                          >
                            Take Test
                        ...
                        <motion.button
                          ...
                          className="flex items-center space-x-1 px-3 py-1 text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors duration-200"
                        ...
                        <motion.button
                          onClick={() => handleDeleteMember(member.enrollmentId)}
                          className="flex items-center space-x-1 px-3 py-1 text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors duration-200"
                          whileHover={{ scale: 1.05 }}
                          whileTap={{ scale: 0.95 }}
                          type="button"
                        >
