...
function AddSets({ex,deleteWorkOut,addWorkOutHandler,deleteBtn,exDetail}) {
  ...
  function handleSaveBtn(){
    const workout={"exerciseId":ex.id,
      "sets":sets
    }
    setIsSaved(true)
        ...
        <div className="flex gap-x-3 my-3 py-1 px-2 items-center rounded-lg border-2 dark:border-slate-500 border-slate-400">
          ...
          <div className="flex flex-row gap-2 items-center">
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                value={form.reps}
                onChange={(e) =>{
                  setForm((prev) => ({ ...prev, reps: e.target.value }))
          ...
          <button
            ...
            onClick={() => {
              ...
              let bool1=form.weight.length === 0 ? false : true;
              let bool2=form.reps.length === 0 ? false : true;
              if(! bool1 && !bool2){
                toast.error("Contain Number Only")
                return ;
              }
              setSets((prev) => [...prev, structuredClone(form)]);
              ...
              // reset percentage selects and formRef values so next add starts fresh
              setPercentWeight('');
              setPercentReps('');
            }}
          >
